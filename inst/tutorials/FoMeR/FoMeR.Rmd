...
file_path <- system.file("extdata", "Relefant_pre3.sav", package = "FoMeR")
...
Wenn wir viele Spalten filtern wollen, kann es praktisch sein statt der einzelnen Spaltennamen ganze Bereiche anzugeben. Mit `Intervention:Dauer` können wir den Bereich von Variable Intervention bis Variable Dauer auswählen, mit `1:5` die Spaltennummern eins bis fünf. Der Doppelpunkt bedeutet also "bis" oder Bindestrich.
...
Erstelle einen Teildatensatz mit allen Kindern älter als 4 Jahre:
...
**Hint:** `relefant_kont <- subset(relefant, Geb > 4)`

</div>

Erstelle einen Teildatensatz mit allen Kindern, für die ein Alter angegeben wurde. D.h. entferne Kinder aus dem Datensatz, die statt einer Zahl "NA" haben.
...
```{r, eval=F, echo=T}
# Bilde einen Teildatensatz von relefant mit allen Kindern der Gruppen (GID) 1 bis 5.
...
relefant_15 <- subset(relefant, GID <= 5)
...
quiz(
  ...
  question("Wie verändern sich Mittelwert, Standardabweichung und das Boxplot durch die Umwandlung von Jahre in Monate?",
    ...
    answer("Die Standardabweichung bleibt gliech"),
    answer("Die Standardabewichungvergrössert sich um den Faktor 12", correct = T),
    answer("Die Proportionen des Boxplot bleiben gleich", correct = T),
...
Du hast alle Kapitel bearbeitet. Viel Erfolg für die Klausur!
